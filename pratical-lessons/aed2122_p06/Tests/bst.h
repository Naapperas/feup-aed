...
int BST<Comparable>::size(const Comparable& el) const {
    ...
    auto elem = this->find(el, this->root);

    if (elem == NULL) return -1;

    if (elem->left == NULL && elem->right == NULL) return 0;
    else {
        int size = 0;
        if (elem->left != NULL) size += this->size(elem->left->element) + 1;
        if (elem->right != NULL) size += this->size(elem->right->element) + 1;
        return size;
    }
}
